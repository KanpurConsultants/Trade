...
#Region "Reports Constant"
    ...
    Private Const NetProfitReport As String = "NetProfitReport"
#End Region
    ...
    Private Sub ProcCurrentStockReport()
        Try
            ...
            mQry = "SELECT I.Code, I.Description, H.LotNo AS LotNo, " & _
                    ...
                    "       I.BillingOn as BillingType,H.DocId As StockInDocID, H.Sr As StockInDocIDSr,    " & _
                    "      (H.Landed_Value/H.Qty_Rec)  as PurchaseRate, (H.Landed_Value/H.Qty_Rec)*(IsNull(H.Qty_Rec, 0) - IsNull(SAdj.AdjQty, 0)) as StockAmount  " & _
                    "     FROM Stock H    " & _
                    "    LEFT JOIN (  " & _
                    "    			SELECT StockInDocID, StockInSr, Sum(AdjQty) AS AdjQty    " & _
                    "    			FROM StockAdj   " & _
                    "    			LEFT JOIN Stock ON StockAdj.StockOutDocID = Stock.DocID AND StockAdj.StockOutSr = Stock.Sr    " & _
                    "    			WHERE 1=1 " & mCondstr1 & _
                    "    			GROUP BY StockInDocID, StockInSr    " & _
                    ...
                    "    Where IsNull(H.Qty_Rec, 0) - IsNull(SAdj.AdjQty, 0) > 0  And IsNull(H.Qty_Rec, 0)>0   " & mCondStr & _
...
#Region "Net Profit Report"
    Private Sub ProcNetProfitReport()
        Try
            RepName = "Med_NetProfitReport" : RepTitle = "Net Profit Report"
            ...
            mQry = "SELECT 1 as Sr, 'Stock Value' as Type, Sum((H.Landed_Value/H.Qty_Rec)*(IsNull(H.Qty_Rec, 0) - IsNull(SAdj.AdjQty, 0))) as Amount  " &
                    "     FROM Stock H    " &
                    "    LEFT JOIN (  " &
                    ...
                    "    Where IsNull(H.Qty_Rec, 0) - IsNull(SAdj.AdjQty, 0) > 0  And IsNull(H.Qty_Rec, 0)>0   " & mCondStr &
                    "  And H.Site_Code ='" & AgL.PubSiteCode & "' "

            mQry += " Union All SELECT 2 as Sr,'Cash' as Type,  sum(H.AmtDr - H.AmtCr  ) as Amount " &
                    " From Ledger H " &
                    " LEFT JOIN subgroup sg ON H.SubCode = sg.SubCode  " &
                    "LEFT JOIN AcGroup ag ON sg.GroupCode = ag.GroupCode  " &
                    "WHERE  ag.Nature ='Cash'   " & mCondStr

            mQry += " Union All SELECT 3 as Sr, 'Bank' as Type,  sum(H.AmtDr - H.AmtCr  ) as Amount " &
                    " From Ledger H " &
                    " LEFT JOIN subgroup sg ON H.SubCode = sg.SubCode  " &
                    "LEFT JOIN AcGroup ag ON sg.GroupCode = ag.GroupCode  " &
                    "WHERE  ag.Nature ='Bank'   " & mCondStr

            mQry += " Union All SELECT 4 as Sr, 'Debtors' as Type,  sum(H.AmtDr - H.AmtCr  ) as Amount " &
                    " From Ledger H " &
                    " LEFT JOIN subgroup sg ON H.SubCode = sg.SubCode  " &
                    "LEFT JOIN AcGroup ag ON sg.GroupCode = ag.GroupCode  " &
                    "WHERE  ag.Nature ='Customer'   " & mCondStr

            mQry += " Union All SELECT 5 as Sr, 'Creditors' as Type,  sum(H.AmtDr - H.AmtCr  ) as Amount " &
                    " From Ledger H " &
                    " LEFT JOIN subgroup sg ON H.SubCode = sg.SubCode  " &
                    "LEFT JOIN AcGroup ag ON sg.GroupCode = ag.GroupCode  " &
                    "WHERE  ag.Nature ='Supplier'   " & mCondStr
